...
class SarsaTable:
    ...
    def learn(self, state, action, reward, new_state, new_action):
        ...
        if new_state != 'terminal':
            q_target = reward + self.gamma * self.q_table.loc[new_state, new_action]  # next state is not terminal
        else:
...
class client_findball3:
        ...
        while not rospy.core.is_shutdown():
            ...
            for i in range(self.episode):
                ...
                while not done:
                    ...
                    action2 = self.Q.choose_action(str(state2))
                    if step == 100:
                        done = True
                        state2 = 'terminal'

                    # Allow for terminal states
                    self.Q.learn(str(state), action, reward, str(state2), action2)
    ...
    def __init__(self):
        ...
        self.episode = 500
        self.Q = SarsaTable(self.action_space)
    ...
    def save_QTable(self,Q_table):
        Q_table.to_csv("/home/miro/mdk/bin/shared/sarsa_qtable.csv")
